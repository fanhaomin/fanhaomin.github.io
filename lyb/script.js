...
AV.init({
  appId: "aixnCfuyDAJ4eBbcPV9cFxMd-gzGzoHsz",
  appKey: "WFQJMC7OVAsFYIiES4wYIyZY",
  serverURL: "https://aixncfuy.lc-cn-n1-shared.com"
});
...
function find() {
    ...
    query.find().then((tests) => {
        tests.forEach((test) => {
          document.querySelector('.tags-cloud').innerHTML += "<li class='tag'><span class='wrap'>"+test.get('words')+"</span></li>";
        });
        main();
    });
}
...
function save() {
    ...
    var kk = document.getElementById('text').value;
    document.querySelector('.tags-cloud').innerHTML=""
if(kk!=""){
    if(kk!=dd){
        testObject.set('words',kk);
        testObject.save().then(testObject)
        dd=kk
        window.alert("您已留言成功")
    }else{
         window.alert("您已留言成功，请勿留言相同内容")
    }
}else{
